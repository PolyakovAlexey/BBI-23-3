...
struct Diver
{
    ...
    {
        double sum = 0; for (int i = 0; i < jump.Length; i++)
        {
...
struct Jump
{
    private int[] scores; private double Coef;
    ...
    public double CalcScore()
    {
        ...
        double sum = 0; for (int i = 1; i < 6; i++)
        {
            sum += scores[i];
        }
        return sum * Coef;
    ...
    static void Main()
    {
        ...
        for (int i = 0; i < divers.Length - 1; i++)
        {
            for (int j = 0; j < divers.Length - i - 1; j++)
            {
                ...
                {
                    Diver temp = divers[j]; divers[j] = divers[j + 1];
                    divers[j + 1] = temp;
                }
            }
        }
        ...
        {
            Console.WriteLine($"{i + 1}\t{divers[i].GetFam()}\t\t{divers[i].GetScore()}");
        }
    }
}