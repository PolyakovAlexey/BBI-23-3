...
class Program
{
    struct PersonOfTheYear
    {
        ...
        private static double TotalVotes; // Статическое поле для хранения общего количества голосов
        ...
        public static void AddVotes(double votes) // метод для добавления голосов
        {
            TotalVotes += votes;
        }

        public static double GetTotalVotes() // метод для получения общего количества голосов
        {
            return TotalVotes;
        }
    }
    ...
    static void Main(string[] args)
    {
        ...
        {
            totalVotes += votes[i];
            PersonOfTheYear.AddVotes(votes[i]);
        }
